...
index_stockname = {}    
i=0
for eachFile in dataset_file:
   index_stockname.update({i:eachFile[0:-7]})         
   i+=1
...
def LMA(index,t,l):
    ...
    return temp
'''
def MAbull(index,t,l,s):
    if LMA(index,t,l) != None and SMA(index,t,s) != None:
        if LMA(index,t,l)- SMA(index,t,s) >5:
            delta = 1
        else:
            delta = 0
    return max(delta,0)

def MAbear(index,t,l=100,s=20):
    if LMA(index,t)- SMA(index,t) >1:
        delta = 1
    else:
        delta = 0
    return max(1-delta,0)

def test(index,l,s):
    middle = len(formatDataset[index])-validLength[index]/2
    #MAbull(123,middle)
    #MAbear(123,middle)
    #if MAbull > MAbear:
    #print middle
    #if LMA(index, middle) > SMA(index,middle):
    try:
        MAbull(index,middle,l,s)
    except ValueError:
        # Not enough data, cannot predict
        return None
    
    if MAbull(index,middle,l,s) == 1:
        #print 'Stock',index,'will rise at the', middle,'! Is this correct from the graph?'
        predict = True
    #elif MAbull < MAbear:
    else:
        #print 'Stock',index,'will drop at the', middle, '! Is this correct from the graph?'
        predict = False
    #else:
     #   print 'Error occurred! MAbull=',MAbull,'MAbear=',MAbear
    #if formatDataset[index][middle+1] > formatDataset[index][middle]:
    if np.mean(formatDataset[index][middle+1:middle+10]) >formatDataset[index][middle]:
        # correct prediction
        return predict
    else:
        # Wrong prediction
...
def get_train_test_data(stock,binary=False):
    #l=300
    #s=30
...
def test(stock):
    ...
    score = []
    training_set,test_set,b_training_label,b_test_label=get_train_test_data(stock,True)
...
def regr(stock,show=False,save=False):
    ...
    training_set,test_set,training_label,test_label = get_train_test_data(stock,False)    
    ...
    '''
    kernel_rgr = KernelRidge(alpha=1.0)
...
#print np.mean([each for [_,each] in scores])
'''
...
# Dump data to file
# Not able to dump the whole dataset
'''
import pickle
with open('formatDataset.txt','wb+') as savefile:
    pickle.dump(formatDataset,savefile)
with open('formatDataset.txt','rb') as savefile:
    formatDataset = pickle.load(savefile)
'''
